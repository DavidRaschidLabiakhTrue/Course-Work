...
char* positive_path = const_cast<char*>("../positive");
char* positive_mask = const_cast<char*>("../positive/positive%04d.pgm");

char* negative_path = const_cast<char*>("../scenery");
char* negative_mask = const_cast<char*>("../scenery/scenery%05d.pgm");
   ...
   // double misclass;
	...
	if(argc>=7 )
		...
		if(!_stricmp(argv[6],"-t"))
			...
			positive_path = const_cast<char*>("../eyes/full/positives/");
			positive_mask = const_cast<char*>("../eyes/full/positives/*.pgm");

			negative_path = const_cast<char*>("../eyes/full/negatives/");
			negative_mask = const_cast<char*>("../eyes/full/negatives/*.pgm");
			}
...
//else
    ...
    for(clas_loop=0;clas_loop < classifier_lvls;clas_loop++) {
		...
		if(tabulate_only)
			{
			tabulate_features(const_cast<char*>("dbase.dat"),feature_set,features_generated,&pos_train[0],pos_count,&neg_train[0],neg_count);
			while(1);
			}
		if(verify_only)
			{
			printf("verification started.....\n");
			verify_read_speed(const_cast<char*>("dbase.dat"));
			while(1);
			}
        ...
        //for positives
        store_weights(pos_train,pos_count, const_cast<char*>("pos-weights.txt"));
        //for negatives
        store_weights(neg_train,neg_count, const_cast<char*>("neg-weights.txt"));
