...
function _14(gl,hex2rgb,colorData,runRender)
{ 
...
function _runRender(transformModel,extents,getModelMatrix,gl,shaders,getPointLightData,getDirectionalLightData,lightingType,getProjectionMatrx,getViewMatrix,m4,lightData2,hex2rgb,colorData,lightData,twgl,buffer){return(
() =>
{
  let renderOffset = transformModel.translationBuffer;
  let renderOffset2 = extents.max[0];

  let finalRenderOffset = transformModel.translationBuffer;
  let renderOffsetArray = [0,0,0]; // precompute center offsets.
  ...
  let model = getModelMatrix(scaleArray, [0,0,0], translateArray, renderOffsetArray);
  for (let i = 0; i < 3; ++i) 
  {
    ...
    {
      case 0:
        renderOffsetArray[0] = -finalRenderOffset;
        break;
      case 1:
        renderOffsetArray[0] = 0;
        break;
      case 2:
        renderOffsetArray[0] = finalRenderOffset;
        break;
    ...
    let viewMatrix = getViewMatrix(renderOffsetArray);
    ...
    let uniforms = 
    {
        ...
        modelMatrix: model,
        materialColor: hex2rgb(colorData.material),
        specularColor: hex2rgb(colorData.specular),
        viewMatrix,
        projectionMatrix,
        shininess: lightData.shininess,
    };
    
    twgl.setUniforms(shaders[i], uniforms);

    buffer.forEach((data) => 
    {
      twgl.setBuffersAndAttributes(gl, shaders[i], data);
...
function _shaders(compile,ErrorDiffuse,diffuseShader,ErrorSpecular,SpecularShader,ErrorDiffuseSpecularAmbient,diffuseSpecularAmbientShader){return(
[compile(ErrorDiffuse, diffuseShader),
          compile(ErrorSpecular, SpecularShader),
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["gl","hex2rgb","colorData","runRender"], _14);
  main.variable(observer("runRender")).define("runRender", ["transformModel","extents","getModelMatrix","gl","shaders","getPointLightData","getDirectionalLightData","lightingType","getProjectionMatrx","getViewMatrix","m4","lightData2","hex2rgb","colorData","lightData","twgl","buffer"], _runRender);
  ...
  main.variable(observer("shaders")).define("shaders", ["compile","ErrorDiffuse","diffuseShader","ErrorSpecular","SpecularShader","ErrorDiffuseSpecularAmbient","diffuseSpecularAmbientShader"], _shaders);
